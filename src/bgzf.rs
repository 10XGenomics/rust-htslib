...
const BGZF_EXTRA: &[u8] = b"BC\x02\x00\x00\x00";
const MIN_FRAME: usize = 20 + BGZF_EXTRA.len();

const MAGIC_BLOCK: &[u8] = b"\x1f\x8b\x08\x04\x00\x00\x00\x00\x00\xff\x06\x00\x42\x43\x02\x00\x1b\x00\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00";
...
fn compressBound(srcLen: usize) -> usize {
	srcLen + srcLen>>12 + srcLen>>14 + srcLen>>25 + 13 + MIN_FRAME
}

fn compress_block(data: &[u8], res: &mut Vec<u8>) -> Result<(), Error>
{
    let b = GzBuilder::new().ex
}
